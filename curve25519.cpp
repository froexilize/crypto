...
static const unsigned char basepoint[CURVE25519_PRIVATE_SZ] = {9};

bool curve25519_gen_keys_pair(curve_public_type &pubkey, curve_private_type &privkey) {
    try {
        seed_type seed;
        memcpy(privkey.data, seed.data, seed_type::get_sz());
        //ed25519_create_seed(privkey);
        privkey.data[0] &= 0xF8;
...
bool curve25519_get_shared_hash(hash_type &shared_hash, const curve_public_type &their_pubkey, const curve_private_type &our_privkey) {
    try {
        curve_shared_type shared_key;// = {0};
        curve25519_donna(shared_key.data, our_privkey.data, their_pubkey.data);
